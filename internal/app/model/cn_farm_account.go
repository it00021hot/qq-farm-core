...
// HasWxAuth reports whether a reusable Yingyongbao login_buffer is stored.
func (a *FarmAccount) HasWxAuth() bool {
	if a == nil {
		return false
	}
	return strings.TrimSpace(a.WxLoginBuffer) != ""
}

// CanWxReconnect is true for WeChat accounts that can mint a new gateway code.
func (a *FarmAccount) CanWxReconnect() bool {
	if a == nil {
		return false
	}
...
// FillWxAuthorized sets API-facing authorization flags (secrets stay json:"-").
func (a *FarmAccount) FillWxAuthorized() {
	if a == nil {
		return
	}
	a.WxAuthorized = a.HasWxAuth()
	a.WxRescanRecommended = a.HasWxAuth() &&
		strings.TrimSpace(a.WxRefreshToken) != "" &&
		a.WxRefreshTokenObservedAt > 0 &&
		time.Now().Unix()-a.WxRefreshTokenObservedAt >= 25*24*60*60
}

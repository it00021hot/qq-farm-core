...
type SysTenant struct {
	...
	MaxUsers     uint   `gorm:"column:max_users;not null;default:0;comment:用户上限 0：不限制" json:"max_users"`
	ExpireAt     uint   `gorm:"column:expire_at;not null;default:0;comment:过期时间Unix秒 0：永不过期" json:"expire_at"`
	ContactName  string `gorm:"column:contact_name;size:64;not null;default:'';comment:联系人" json:"contact_name"`
	ContactPhone string `gorm:"column:contact_phone;size:32;not null;default:'';comment:联系电话" json:"contact_phone"`
...
type SysAdminTenant struct {
	ID        uint64 `gorm:"column:id;primaryKey;autoIncrement;comment:主键ID" json:"id"`
	AdminID   uint64 `gorm:"column:admin_id;not null;uniqueIndex:uk_admin_tenant;comment:平台用户ID" json:"admin_id"`
	TenantID  uint64 `gorm:"column:tenant_id;not null;uniqueIndex:uk_admin_tenant;index;comment:可管理的租户ID" json:"tenant_id"`
	CreatedAt uint   `gorm:"column:created_at;not null;comment:创建时间" json:"created_at"`
}

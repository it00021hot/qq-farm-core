...
func DefaultAccountConfig() AccountConfig {
	return AccountConfig{
		Automation: AutomationConfig{
			...
			// 以下默认值按大号（小树）的实际自动化配置对齐（2026-09-15）：
			...
			Task:                               true,
			FertilizerGift:                     true,
			FertilizerBuyOrganic:               true,
			FertilizerBuyNormal:                true,
			MysteryShopAutoBuy:                 true,
			MysteryShopAllowGold:               true,
			MysteryShopAllowCoupon:             true,
			MysteryShopAllowGoldBean:           true,
			MysteryShopAllowDiamond:            true,

package mallpb

import (
	"fmt"

	"google.golang.org/protobuf/encoding/protowire"
)

func skipField(num protowire.Number, typ protowire.Type, data []byte) ([]byte, error) {
	n := protowire.ConsumeFieldValue(num, typ, data)
	if n < 0 {
		return nil, fmt.Errorf("mallpb: bad field %d typ=%d", num, typ)
	}
	return data[n:], nil
}

func appendInt32(b []byte, field int, v int32) []byte {
	if v == 0 {
		return b
	}
	b = protowire.AppendTag(b, protowire.Number(field), protowire.VarintType)
	b = protowire.AppendVarint(b, uint64(v))
	return b
}

func appendInt64Varint(b []byte, field int, v int64) []byte {
	if v == 0 {
		return b
	}
	b = protowire.AppendTag(b, protowire.Number(field), protowire.VarintType)
	b = protowire.AppendVarint(b, uint64(v))
	return b
}

func appendBool(b []byte, field int, v bool) []byte {
	if !v {
		return b
	}
	b = protowire.AppendTag(b, protowire.Number(field), protowire.VarintType)
	b = protowire.AppendVarint(b, 1)
	return b
}

func appendString(b []byte, field int, v string) []byte {
	if v == "" {
		return b
	}
	b = protowire.AppendTag(b, protowire.Number(field), protowire.BytesType)
	b = protowire.AppendString(b, v)
	return b
}

func appendBytes(b []byte, field int, v []byte) []byte {
	if len(v) == 0 {
		return b
	}
	b = protowire.AppendTag(b, protowire.Number(field), protowire.BytesType)
	b = protowire.AppendBytes(b, v)
	return b
}

func appendMessage(b []byte, field int, raw []byte) []byte {
	if len(raw) == 0 {
		return b
	}
	b = protowire.AppendTag(b, protowire.Number(field), protowire.BytesType)
	b = protowire.AppendBytes(b, raw)
	return b
}

// ParseMallPrice reads field 2 varint from serialized price bytes.
func ParseMallPrice(price []byte) int64 {
	var parsed int64
	for len(price) > 0 {
		num, typ, n := protowire.ConsumeTag(price)
		if n < 0 {
			break
		}
		price = price[n:]
		if num == 2 && typ == protowire.VarintType {
			v, m := protowire.ConsumeVarint(price)
			if m < 0 {
				break
			}
			price = price[m:]
			parsed = int64(v)
			continue
		}
		var err error
		price, err = skipField(num, typ, price)
		if err != nil {
			break
		}
	}
	if parsed < 0 {
		return 0
	}
	return parsed
}

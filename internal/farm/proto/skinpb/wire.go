...
func skipField(num protowire.Number, typ protowire.Type, data []byte) ([]byte, error) {
	...
	if n < 0 {
		return nil, fmt.Errorf("skinpb: bad field %d typ=%d", num, typ)
	}
	return data[n:], nil
}
...
func consumeRepeatedInt64(num protowire.Number, typ protowire.Type, data []byte, dst *[]int64) ([]byte, error) {
	...
	case protowire.BytesType:
		...
		if n < 0 {
			return nil, fmt.Errorf("skinpb: bad packed int64 field %d", num)
		}
		for len(raw) > 0 {
	...
	case protowire.VarintType:
		...
		if n < 0 {
			return nil, fmt.Errorf("skinpb: bad int64 field %d", num)
		}
		*dst = append(*dst, int64(v))
		return data[n:], nil
	default:

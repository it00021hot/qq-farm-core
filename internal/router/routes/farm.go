...
func InitFarmGroup(r fiber.Router, handles ...any) {
	router := r.Group("farm", handles...)
	{
		...
		router.Get("/system/qqbot/bind", status.Status.QqBotBindStatus).Name("QQ机器人绑定状态")
		router.Post("/system/qqbot/bind/start", status.Status.QqBotBindStart).Name("发起QQ机器人绑定")
		...
		router.Post("/friend/delete", friend.Friend.Delete).Name("删除好友")
		router.Get("/friend/interaction-items", friend.Friend.InteractionItems).Name("互动道具列表")
		router.Post("/friend/interaction-use", friend.Friend.InteractionUse).Name("互动道具使用")
		router.Get("/pets/dog-info", friend.Friend.DogInfo).Name("宠物信息")
		router.Post("/pets/dog-op", friend.Friend.DogOperate).Name("宠物操作")
		router.Get("/pets/protect-logs", friend.Friend.DogProtectLogs).Name("守护记录")
		router.Get("/illustrated/snapshot", friend.Friend.IllustratedSnapshot).Name("图鉴快照")
		...
		router.Get("/activity/charity", activity.Activity.Charity).Name("公益小红花快照")
		router.Post("/activity/charity/operate", activity.Activity.CharityOperate).Name("公益小红花操作")
		router.Get("/activity/weather", activity.Activity.Weather).Name("雨落成诗快照")
